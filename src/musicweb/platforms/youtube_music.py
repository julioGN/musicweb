...
class YouTubeMusicParser(BasePlatformParser):
    ...
    def parse_file(self, file_path: str) -> Library:
        ...
        library = Library(
            f"YouTube Music Library from {file_path.name}", "youtube_music"
        )
    ...
    def _json_item_to_track(self, item: Dict[str, Any]) -> Optional[Track]:
        ...
        try:
            ...
            artist = (
                item.get("artist") or item.get("channel") or item.get("uploader", "")
            )
    ...
    def _parse_csv_manual(
        self, file_path: str, encoding: str, library: Library
    ) -> Library:
    ...
    if file_path.suffix.lower() == ".json":
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                if (
                    '"platform":"spotify"' in sample
                    or '"platform": "spotify"' in sample
                ):
                    return "spotify"
...
def create_parser(platform: str) -> BasePlatformParser:
    ...
    if platform in ("apple music xml", "apple xml", "apple_music_xml", "amxml"):
        from .apple_music import AppleMusicXMLParser
