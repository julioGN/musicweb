...
class TrackMatcher:
    ...
    def find_best_match_optimized(
        self, source_track: Track, candidates: List[Track]
    ...
    def _filter_candidates_by_words(
        self, source_track: Track, candidates: List[Track]
    ) -> List[Track]:
        ...
        if potential_matches:
            ...
            return (
                filtered if filtered else candidates[: min(100, len(candidates))]
            )  # Limit fallback
    ...
    def calculate_match(self, track1: Track, track2: Track) -> Optional[MatchResult]:
        ...
        if (
            ...
            and track1.isrc
            and track2.isrc
            and track1.isrc.strip().upper() == track2.isrc.strip().upper()
        ):
            return MatchResult(
                source_track=track1,
                target_track=track2,
        ...
        if self.enable_duration and track1.duration and track2.duration:
            ...
            if duration_diff <= self.duration_tolerance:
                ...
                similarity_scores["duration"] = 1 - (
                    duration_diff / self.duration_tolerance
                )
    ...
    def _calculate_artist_similarity(self, track1: Track, track2: Track) -> float:
        ...
        string_similarity = self._calculate_similarity(
            track1.normalized_artist, track2.normalized_artist
        )
