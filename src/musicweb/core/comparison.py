...
from ..core.models import Library, Track, TrackMatcher
...
class ComparisonResult:
    ...
    def __post_init__(self):
        ...
        if self.matches:
            ...
            # Calculate averages
            self.avg_confidence = sum(m.confidence for m in self.matches) / len(
                self.matches
            )
    ...
    def save_results(
        self, output_dir: str, base_filename: str = None
    ) -> Dict[str, str]:
        ...
        if not base_filename:
            source_clean = (
                self.source_library.lower().replace(" ", "_").replace("-", "_")
            )
            target_clean = (
                self.target_library.lower().replace(" ", "_").replace("-", "_")
            )
        ...
        if self.missing_tracks:
            missing_file = (
                output_path / f"{base_filename}_missing_from_{target_clean}.csv"
            )
...
class LibraryComparator:
    ...
    def compare_libraries(
        self, source_library: Library, target_library: Library
    ) -> ComparisonResult:
        ...
        for source_track in source_music:
            if self.progress_callback:
                self.progress_callback(
                    processed, len(source_music), f"Processing: {source_track.title}"
                )
    ...
    def _find_match(
        ...
        target_by_base: Dict[Tuple[str, str], List[Track]],
        ...
        if exact_candidates:
            ...
            if len(exact_candidates) > 1:
                ...
                for candidate in exact_candidates:
                    candidate_confidence = self.matcher.calculate_match_confidence(
        ...
        if base_candidates:
            ...
            for candidate in base_candidates:
                ...
                if score > best_confidence:
                    ...
                    best_candidate = candidate
            if best_candidate and best_confidence >= (
                0.82 if self.matcher.strict_mode else 0.75
            ):
                return MatchResult(
                    source_track=source_track,
                    target_track=best_candidate,
    ...
    def _find_unique_tracks(
        ...
        for i, library in enumerate(libraries):
            ...
            for track in library.music_tracks:
                ...
                for j, other_library in enumerate(libraries):
                    ...
                    for other_track in other_library.music_tracks:
                        other_key = (
                            other_track.normalized_title,
                            other_track.normalized_artist,
                        )
            ...
            unique_tracks[library.name] = sorted(
                library_unique, key=lambda x: (x["artist"], x["title"])
            )
    ...
    def _analyze_artists(self, libraries: List[Library]) -> Dict[str, Any]:
        ...
        artist_analysis["library_artists"] = {
            name: len(artists) for name, artists in library_artists.items()
        }
        ...
        artist_analysis["top_overlap_artists"] = sorted(
            artist_totals.items(), key=lambda x: x[1], reverse=True
        )[:20]
