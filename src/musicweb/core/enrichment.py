...
class MusicBrainzEnricher:
    ...
    def enrich_track(self, track: Track) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not enrichment_data:
                enrichment_data = self._search_by_artist_title(
                    track.artist, track.title
                )

            # 3. Search with album if available
            if not enrichment_data and track.album:
                enrichment_data = self._search_by_artist_title_album(
    ...
    def enrich_tracks(
        self, tracks: List[Track], progress_callback: Optional[callable] = None
        ...
        try:
            ...
            for query in queries:
                ...
                if recordings:
                    ...
                    best_match = self._find_best_recording_match(
    ...
    def _search_by_artist_title_album(
    ...
    def _calculate_recording_similarity(
        self,
        recording: Dict,
        target_artist: str,
        target_title: str,
        ...
        try:
            ...
            if target_album:
                ...
                for release in releases:
                    ...
                    album_similarity = self._simple_similarity(
                        mb_album, target_album.lower()
                    )
    ...
    def _extract_recording_info(self, recording: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            for tag in recording.get("tags", []):
                tags.append({"name": tag.get("name"), "count": tag.get("count", 0)})
    ...
    def enrich_track(self, track: Track) -> Dict[str, Any]:
        ...
        if mb_data:
            ...
            if "length" in mb_data and mb_data["length"] and not track.duration:
                enrichment_data["enriched_fields"]["duration"] = (
                    mb_data["length"] // 1000
                )
