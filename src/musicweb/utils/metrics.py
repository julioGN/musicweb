...
class MetricsCollector:
    ...
    def record_timer(self, name: str, duration: float, tags: Dict[str, str] = None):
        ...
        with self._lock:
            ...
            self._timers[key].append(
                {"timestamp": datetime.now(), "duration": duration}
            )
    ...
    def record_histogram(self, name: str, value: float, tags: Dict[str, str] = None):
        ...
        with self._lock:
            key = self._make_key(name, tags)
            self._metrics[key].append({"timestamp": datetime.now(), "value": value})
            ...
            return {
                ...
                "timers": {
                    k: self.get_timer_stats(
                        k.split("|")[0], self._parse_tags(k.split("|")[1:])
                    )
...
def export_metrics_to_prometheus(file_path: str = None):
    ...
    for name, stats in all_metrics["timers"].items():
        ...
        prometheus_metrics.append(
            f"musicweb_{clean_name}_duration_seconds {stats['avg']}"
        )
