...
def validate_library_data(library: Library) -> Dict[str, Any]:
    ...
    if tracks_with_issues > 0:
        ...
        if percentage > 10:
            report["issues"].append(
                f"{tracks_with_issues} tracks have validation issues ({percentage:.1f}%)"
            )
            report["valid"] = False
        else:
            report["warnings"].append(
                f"{tracks_with_issues} tracks have minor issues ({percentage:.1f}%)"
            )
    ...
    if duplicates > 0:
        ...
        if percentage > 5:
            report["warnings"].append(
                f"{duplicates} potential duplicate tracks ({percentage:.1f}%)"
            )
...
def validate_comparison_parameters(
    strict_mode: bool, enable_duration: bool, enable_album: bool
) -> List[str]:
