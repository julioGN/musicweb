...
from musicweb import Library, LibraryComparator, Track, create_parser, detect_platform
...
try:
    from musicweb.integrations.youtube_music import YTMusicCleaner
except ImportError:
    YTMusicCleaner = None
...
def load_library(file_path: str, platform: str = None) -> Library:
    ...
    print(
        f"Loaded {library.name}: {library.music_count} music tracks, {library.non_music_count} non-music"
    )
    return library
...
def create_playlist_command(args):
    ...
    if result["success"]:
        ...
        print(
            f"  Success rate:   {result['total_added']/result['total_requested']:.1%}"
        )
        ...
        if result["failed_tracks"]:
            ...
            if args.save_failed:
                failed_file = (
                    ...
                    / f"failed_tracks_{int(time.time())}.json"
                )
...
def analyze_command(args):
    ...
    for lib_stats in analysis["libraries"]:
        print(
            f"  {lib_stats['name']}: {lib_stats['music_tracks']:,} tracks, {lib_stats['unique_artists']:,} artists"
        )

    print(f"\n🔄 Cross-Platform Analysis:")
    print(f"  Universal tracks: {len(analysis['universal_tracks']):,}")
    print(
        f"  Total unique artists: {analysis['artist_analysis']['total_unique_artists']:,}"
    )
    print(
        f"  Universal artists: {len(analysis['artist_analysis']['universal_artists']):,}"
    )
    ...
    for comparison in analysis["pairwise_comparisons"]:
        ...
        print(
            f"  {comparison['source']} vs {comparison['target']}: {stats['match_rate']:.1f}% match rate"
        )
...
def enrich_command(args):
    ...
    print(
        f"  Successfully enriched: {successful:,} ({successful/len(enriched_results):.1%})"
    )
...
def dedup_youtube_music_command(args):
    ...
    if not args.headers or not Path(args.headers).exists():
        print(
            "❌ Error: headers_auth.json is required (generate with 'ytmusicapi setup')"
        )
        return None
...
def clean_youtube_music_command(args):
    ...
    print(
        f"  Total Adds:         {sum(len(e.add_video_ids) for e in plan.playlist_edits)}"
    )
    print(
        f"  Total Removes:      {sum(len(e.remove_items) for e in plan.playlist_edits)}"
    )
    ...
    if not args.dry_run:
        ...
        print(
            f"✅ Done — Unliked: {summary['unliked']}, Adds: {summary['playlist_adds']}, Removes: {summary['playlist_removes']}"
        )
        if summary["errors"]:
...
def rollback_youtube_music_command(args):
    ...
    print(
        f"✅ Rolled back — Re-added: {res['playlist_readds']}, Removed Winners: {res['playlist_add_removals']}, Reliked: {res['ratings_liked']}"
    )
    if res["errors"]:
...
Examples:
  ...
  python musictools.py enrich --library apple_music.csv --output-dir enriched/
    ...
    compare_parser = subparsers.add_parser(
        "compare", help="Compare two music libraries"
    )
    ...
    compare_parser.add_argument(
        "--source-platform", help="Source platform (auto-detect if not specified)"
    )
    compare_parser.add_argument(
        "--target-platform", help="Target platform (auto-detect if not specified)"
    )
    compare_parser.add_argument(
        "--strict", action="store_true", help="Use strict matching (higher precision)"
    )
    compare_parser.add_argument(
        "--no-duration", action="store_true", help="Disable duration matching"
    )
    compare_parser.add_argument(
        "--use-album", action="store_true", help="Enable album matching"
    )

    # Create playlist command
    playlist_parser = subparsers.add_parser(
        "create-playlist", help="Create YouTube Music playlist"
    )
    playlist_parser.add_argument(
        "--tracks", required=True, help="Tracks file (CSV or library file)"
    )
    playlist_parser.add_argument("--name", help="Playlist name")
    playlist_parser.add_argument(
        "--headers", required=True, help="YouTube Music headers file"
    )
    playlist_parser.add_argument(
        "--search-fallback",
        action="store_true",
        help="Use search fallback for missing tracks",
    )
    playlist_parser.add_argument(
        "--validate",
        action="store_true",
        help="Validate tracks before creating playlist",
    )
    playlist_parser.add_argument(
        "--force", action="store_true", help="Force creation even with low success rate"
    )
    playlist_parser.add_argument(
        "--save-failed", action="store_true", help="Save failed tracks to file"
    )

    # Analyze command
    analyze_parser = subparsers.add_parser("analyze", help="Analyze multiple libraries")
    analyze_parser.add_argument(
        "--libraries", nargs="+", required=True, help="Library files to analyze"
    )
    analyze_parser.add_argument(
        "--strict", action="store_true", help="Use strict matching"
    )
    analyze_parser.add_argument(
        "--no-duration", action="store_true", help="Disable duration matching"
    )
    ...
    enrich_parser = subparsers.add_parser(
        "enrich", help="Enrich metadata using MusicBrainz"
    )
    enrich_parser.add_argument(
        "--library", required=True, help="Library file to enrich"
    )
    enrich_parser.add_argument(
        "--limit", type=int, help="Limit number of tracks to enrich"
    )

    # YouTube Music deduplication
    dedup_parser = subparsers.add_parser(
        "dedup-ytm", help="Find duplicates in your YouTube Music library"
    )
    dedup_parser.add_argument(
        "--headers", required=True, help="Path to headers_auth.json (ytmusicapi)"
    )
    dedup_parser.add_argument(
        "--limit", type=int, help="Limit number of library songs to scan"
    )
    dedup_parser.add_argument(
        "--threshold",
        type=float,
    ...
    dedup_parser.add_argument(
        "--output-dir", "-o", help="Directory to export duplicate report JSON"
    )
    dedup_parser.add_argument(
        "--create-playlist",
        action="store_true",
        help="Create a playlist with detected duplicates",
    )
    dedup_parser.add_argument(
        "--playlist-name", help="Name for the duplicates playlist"
    )
    dedup_parser.add_argument(
        "--prefer-explicit",
        action="store_true",
        help="Prefer explicit version when picking the one to keep",
    )
    dedup_parser.add_argument(
        "--losers-only",
        action="store_true",
        help="Add only non-preferred items from each group to the playlist",
    )
    dedup_parser.add_argument(
        "--winners-only",
        action="store_true",
        help="Add only the preferred item from each group to the playlist",
    )

    # Cleanup command
    clean_parser = subparsers.add_parser(
        "clean-ytm",
        help="Plan and apply cleanup: unlike losers and replace in playlists",
    )
    ...
    clean_parser.add_argument(
        "--prefer-explicit",
        action="store_true",
        help="Prefer explicit version as the one to keep",
    )
    clean_parser.add_argument(
        "--groups", help="Comma-separated group IDs to include (default: all)"
    )
    clean_parser.add_argument(
        "--unlike-losers",
        action="store_true",
        help="Unlike loser tracks from your library",
    )
    clean_parser.add_argument(
        "--replace-in-playlists",
        action="store_true",
        help="Replace losers with winners in your playlists",
    )
    clean_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Do not apply changes; just print summary",
    )
    clean_parser.add_argument("--save-plan", help="Path to save cleanup plan JSON")
    clean_parser.add_argument(
        "--save-undo", help="Path to save undo log JSON (for rollback)"
    )

    rollback_parser = subparsers.add_parser(
        "rollback-ytm", help="Rollback a previous cleanup using an undo log"
    )
    rollback_parser.add_argument(
        "--headers", required=True, help="Path to headers_auth.json (ytmusicapi)"
    )
    rollback_parser.add_argument(
        "--undo-log", required=True, help="Path to undo log JSON produced by clean-ytm"
    )
