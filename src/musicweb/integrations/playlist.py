    ...
    def search_track(self, track: Track, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if "Expecting value" in str(e):
                print(
                    f"JSON parsing error - possible empty response from YouTube Music API"
                )
            return []
    ...
    def create_playlist(
        ...
        try:
            ...
            if isinstance(response, dict):
                ...
                raise Exception(
                    f"Unexpected response type from YouTube Music API: {type(response)}, content: {response}"
                )
            ...
            for i in range(0, len(tracks), batch_size):
                ...
                if progress_callback:
                    progress_callback(
                        i, len(tracks), f"Processing batch {i//batch_size + 1}"
                    )
                ...
                if not batch_results:
                    failed_tracks.extend(
                        [
                            {"track": t.to_dict(), "reason": "Batch processing failed"}
                            for t in batch
                        ]
                    )
                else:
            ...
            if progress_callback:
                progress_callback(
                    len(tracks), len(tracks), "Playlist creation complete"
                )
            ...
            return {
                ...
                "added_tracks": [],
                "failed_tracks": [
                    {"track": t.to_dict(), "reason": "Playlist creation failed"}
                    for t in tracks
                ],
            }

    def _add_tracks_to_playlist(
        self, playlist_id: str, tracks: List[Track], search_fallback: bool
    ) -> Dict[str, List]:
            ...
            try:
                ...
                if match and "youtube_track" in match:
                    ...
                    if video_id:
                        ...
                        failed_tracks.append(
                            {"track": track.to_dict(), "reason": "No video ID found"}
                        )
                else:
                    failed_tracks.append(
                        {"track": track.to_dict(), "reason": "No suitable match found"}
                    )
                ...
                failed_tracks.append({"track": track.to_dict(), "reason": str(e)})
...
class PlaylistAnalyzer:
    ...
    def validate_tracks(self, tracks: List[Track]) -> Dict[str, Any]:
        ...
        for track in tracks:
            ...
            if issues:
                invalid_tracks.append({"track": track, "issues": issues})
            else:
    ...
    def analyze_playlist_potential(self, tracks: List[Track]) -> Dict[str, Any]:
        ...
        avg_confidence = sum(r["best_match_confidence"] for r in match_results) / len(
            match_results
        )
        ...
        return {
            ...
            "recommendation": self._get_recommendation(
                good_matches / sample_size, avg_confidence
            ),
        }
